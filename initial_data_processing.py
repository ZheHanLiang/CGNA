...
def data_processing(args):
    ...
    node_set = set() # 记录被读取到的节点（方便后续补充中间被遗漏的孤岛节点）
    nodes_beyond_degree = set() # 记录满足degree要求的节点
    can_discard_list = [] # 记录可以删除的边的序号
    present_edge_list = [] # 当前节点所连接的边的序号
    f = open(output_new_data_dir, 'r', encoding='utf-8') # 第一次遍历文件，找出度满足要求的节点，并记录所有读取到的节点以及原图边的总数
    for line in f.readlines():
        ...
        if int(nodes[0])==present_node: # 还是同个点时；因为是从start_node开始记录的，所以start_node肯定是第一个被选择的节点
            ...
            # present_edge_list.append(index_edge) # 把当前边加入当前节点所连接的边的list中
        node_set.add(int(nodes[0])) # 添加被读取到的节点
        node_set.add(int(nodes[1])) # 添加被读取到的节点
        index_edge += 1 # 更新当前边的序号
    f.close()
    num_edge = index_edge # 记录原图总边数
    index_edge = 0 # 边的序号
    f = open(output_new_data_dir, 'r', encoding='utf-8') # 第二次遍历文件，根据度满足要求的节点确定能够删除的边的数目与序号
    for line in f.readlines():
        nodes = line.strip().split(' ') # 依次读入每行数据
        if nodes[0]!=nodes[1] and int(nodes[0]) in nodes_beyond_degree and int(nodes[1]) in nodes_beyond_degree: # 自连接不纳入可以删除的边
    ...
    assert num_high_degree_edge>2*num_edge_discard, "Edges aren't enough to discard!" # 验证是否有足够的边可以删除
