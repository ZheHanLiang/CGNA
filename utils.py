...
# @Author: Zhehan Liang
# @Date: 6/8/2020
# @Intro: Some tool functions used
##########################################################################
...
def get_node_num(params):
    """
    Read node num from data path
    """
    path_list = os.listdir(params.data_path) # Get the file name under the data path
    for file_name in path_list:
        if ".edge" in file_name and params.dataset in file_name: # Find the .edge file corresponding to the dataset
            num = int(file_name.split("_")[1]) # Parse the file name, the number after the first '_' in the file name is the number of nodes
...
def get_metric_matrix(matrix1, matrix2, method):
    ...
    metric_matrix = metric_matrix - (np.tile(mm_1, (len(matrix1), 1)).T + np.tile(mm_2, (len(matrix1), 1))) * 0.4 # Then calculate the metric matrix under the DCIS strategy
